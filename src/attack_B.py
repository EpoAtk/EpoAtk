...
def method_add(x, y, new_adj_tensor_cuda, new_surrogate_model):
    ...
    loss_test = -new_surrogate_model.margin_loss
...
def get_greedy_list(ori_adj_cuda, Greedy_edges, change_edges):
    ...
    loss_entropy = -torch.sum(torch.mul(logits, torch.log(logits)))
    loss = F.nll_loss(outputs[idx_train_cuda], _z_cuda[idx_train_cuda])
    '''

    outputs = new_surrogate_model(_X_cuda, new_adj_norm_tensor_cuda, False, drop_rate=args.drop_rate)

    t_pre = _z_cuda[idx_train_cuda]
    pre = torch.exp(outputs[idx_train_cuda])
    sel = pre[torch.arange(pre.size(0)), t_pre]

    class_eye = torch.eye(_K.item()).cuda()
    _z_one_hot = class_eye[_z_cuda][idx_train_cuda]
    pre = pre * (1 - _z_one_hot)

    max_pre = torch.min(pre, 1)[0]
    loss = torch.sum(max_pre - sel)
    ...
    for i in range(len(co_list)):
        ...
        if (x, y) in changes or (y, x) in changes:
            ...
            new_model.model_test(_X_cuda, adj_norm_tensor_cuda, pre_all_labels_cuda, extra_idx_cuda,
                                        use_relu=False)
            #loss_test = -new_target_model.loss_test
            loss_test = -new_model.margin_loss
